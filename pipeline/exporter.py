...
def export_events(cursor):
    ...
    for row in cursor.fetchall():
        ...
        for occ in cursor.fetchall():
            ...
            if (
                start_date
                and start_date <= future_limit_date
    ...
    for event in all_events:
        ...
        is_in_bbox = (
            lat is not None
            and lng is not None
            and INIT_LAT_RANGE[0] <= lat <= INIT_LAT_RANGE[1]
            and INIT_LNG_RANGE[0] <= lng <= INIT_LNG_RANGE[1]
        )
        ...
        if first_occurrence_start_str:
            try:
                start_date = datetime.strptime(
                    first_occurrence_start_str, "%Y-%m-%d"
                ).date()
    ...
    full_locations = [
        loc
        for loc in get_active_locations(full_events, all_locations)
    ...
    print(
        f"  Exported {len(init_locations)} init locations, {len(full_locations)} full locations"
    )
