...
def resolve_locations(
    raw_json_data: Any, website_id: int, cursor: Any, connection: Any
) -> int:
    ...
    existing_locations: list[dict[str, Any]] = db.get_all_locations(cursor)
    ...
    for lugar_id, venue in unique_venues.items():
        ...
        if not (
            BA_BOUNDS["lat_min"] <= venue_lat <= BA_BOUNDS["lat_max"]
            and BA_BOUNDS["lng_min"] <= venue_lng <= BA_BOUNDS["lng_max"]
        ):
            skipped_bounds += 1
            continue

        # Name normalization
        display_name: str = normalize_event_name_caps(venue["nombre"])
        normalized_name: str = _normalize_location_name(display_name)
        ...
        for existing_lat, existing_lng in existing_coords:
            if (
                _haversine_meters(venue_lat, venue_lng, existing_lat, existing_lng)
        ...
        address_parts: list[str] = [venue["direccion"]]
        if venue["zona"]:
            address_parts.append(venue["zona"])
        address: str = ", ".join(p for p in address_parts if p)
        ...
        cursor.execute(
            """INSERT INTO locations (name, address, lat, lng, emoji)
               VALUES (%s, %s, %s, %s, %s)
               RETURNING id""",
            (display_name, address, venue_lat, venue_lng, "\U0001f3ad"),
        )
        new_location_id: int = cursor.fetchone()[0]
        ...
        existing_coords.append((venue_lat, venue_lng))
