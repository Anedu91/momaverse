...
def get_significant_words(name: str, stem: bool = False) -> set[str]:
    ...
    def is_year(w: str) -> bool:
        """Check if word is a 4-digit year (2000-2099)."""
        return len(w) == 4 and w.isdigit() and w.startswith("20")

    result = set(
        w for w in words if len(w) >= 3 and w not in stop_words and not is_year(w)
    )
    if stem:
...
def merge_crawl_events(
    cursor: Any, connection: Any, crawl_run_id: int | None = None
) -> tuple[int, int]:
    """
    ...
    future_limit_date: date = (datetime.now() + timedelta(days=90)).date()

    # Initialize edit logger if available
    edit_logger: Any = None
    if EditLogger:
    ...
    recent_cutoff: date = (datetime.now() - timedelta(days=10)).date()
    cursor.execute(
        """
    ...
    existing_events_by_coords: dict[
        tuple[float, float], list[dict[str, Any]]
    ] = {}  # key: (lat, lng) -> list of {id, name}
    existing_events_by_location_id: dict[
        int, list[dict[str, Any]]
    ] = {}  # key: location_id -> list of {id, name}
    existing_events_by_location: dict[
        str, list[dict[str, Any]]
    ] = {}  # key: normalized location_name -> list of {id, name}
    existing_events_by_website: dict[
        int, list[dict[str, Any]]
    ] = {}  # key: website_id -> list of {id, name}
    event_ids_with_future: set[int] = set()
    ...
    event_dates: dict[int, set[str]] = {eid: set() for eid in event_ids_with_future}
    ...
    for ce_row in new_crawl_events:
        ...
        norm_name: str = normalize_name_for_dedup(name)

        def find_best_match(candidates: list[dict[str, Any]]) -> int | None:
                ...
                if crawl_event_dates & existing_dates:  # date overlap required
                    if are_names_similar(name, existing["name"]):
                        if normalize_name_for_dedup(existing["name"]) == norm_name:
                            return int(existing["id"])  # Exact match — best possible
        ...
        if matched_event_id:
            ...
            if location_id:
                cursor.execute(
                    "SELECT location_id FROM events WHERE id = %s", (matched_event_id,)
                )
                result = cursor.fetchone()
                current_location_id = result[0] if result else None
                if not current_location_id:
                    cursor.execute(
                        "UPDATE events SET location_id = %s WHERE id = %s",
