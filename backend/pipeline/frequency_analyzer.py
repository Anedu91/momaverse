...
def _compute_lead_times(cursor: Any, website_id: int) -> list[int]:
    """
...
def _compute_event_horizon(cursor: Any, website_id: int) -> list[int]:
    """
...
def _compute_new_event_rate(cursor: Any, website_id: int) -> dict[str, Any]:
    """
    ...
    total: int = len(crawls)
    with_new: int = sum(1 for c in crawls if c["new_events"] > 0)
...
def _compute_stability(new_event_data: dict[str, Any]) -> dict[str, int]:
    """
...
def _has_upcoming_events(cursor: Any, website_id: int) -> bool:
...
def _recommend_frequency(
    ...
    """
    current: int = current_frequency or DEFAULT_FREQUENCY
    reasons: list[str] = []

    # Compute P25 values
    p25_lead_time: int | None = _percentile(lead_times, LEAD_TIME_PERCENTILE)
    p25_horizon: int | None = _percentile(horizons, LEAD_TIME_PERCENTILE)

    # Frequency from lead time signal
    freq_from_lead: int | None = None
    ...
    freq_from_horizon: int | None = None
    ...
    if stability["consecutive_no_new"] >= STALE_CRAWL_THRESHOLD:
        adjusted: int = int(recommended * STALE_FREQUENCY_MULTIPLIER)
    ...
    # Limit change rate
    if current > 0:
        max_new: int = int(current * MAX_CHANGE_FACTOR)
        min_new: int = max(MIN_FREQUENCY, int(current / MAX_CHANGE_FACTOR))
...
def analyze_frequencies(
    ...
    if website_ids:
        placeholders: str = ",".join(["%s"] * len(website_ids))
        cursor.execute(
            f"""
    ...
    for w in websites:
        wid: int = w["id"]
        name: str = w["name"]
        current_freq: int = w["crawl_frequency"] or DEFAULT_FREQUENCY
        ...
        crawl_count: int = w["crawl_count"]
        if crawl_count < MIN_CRAWL_HISTORY:
            if verbose:
                print(
                    f"  Skipped {name}: only {crawl_count} crawls (need {MIN_CRAWL_HISTORY})"
                )
            skipped += 1
            continue
        ...
        analyzed += 1
        new_freq: int = recommendation["frequency"]
        ...
        if verbose and (
            ...
            m: dict[str, Any] = recommendation["metrics"]
