...
def flatten_events_to_markdown(
    events_dict: dict[str, Any], fields_include: list[str] | None = None
) -> str:
...
async def crawl_json_api(
    website: dict[str, Any], cursor: Any, connection: Any, crawl_run_id: int
) -> tuple[int | None, Any]:
...
def create_safe_filename(name: str, extension: str | None = None) -> str:
