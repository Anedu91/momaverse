...
def extract_image_urls(content: str, base_url: str | None = None) -> list[str]:
    """
...
async def download_and_encode_image(
    url: str, max_dimension: int = MAX_IMAGE_DIMENSION
...
async def prepare_vision_content(
    content: str, base_url: str | None = None, max_images: int = MAX_VISION_IMAGES
...
def get_vision_prompt(
    url: str, text_content: str, current_date_string: str, name: str, notes: str
) -> str:
...
def chunk_content_by_events(
    content: str, events_per_chunk: int = EVENTS_PER_CHUNK
) -> list[str]:
    """
...
def chunk_content_by_size(content: str, max_chars: int = MAX_CHUNK_CHARS) -> list[str]:
    """
...
def count_event_markers(content: str) -> int:
...
def estimate_event_count(content: str) -> int:
    """
...
def get_enrichment_prompt(event_names: list[str], venue_name: str) -> str:
...
async def enrich_events_batch(
    event_names: list[str], venue_name: str
...
async def extract_chunk(
    chunk_content: str, current_date_string: str, notes: str
) -> list[dict[str, Any]]:
    """
...
async def extract_large_page(
    ...
    all_simple_events: list[dict[str, Any]] = []
    skipped_chunks = 0
...
async def extract_events(
    ...
    url = url or ""
    content_to_process = content_to_process or page_content
