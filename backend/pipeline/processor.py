...
def strip_leading_emoji(text: str) -> str:
    ...
    result: str = emoji_pattern.sub("", text)
    return result
...
def create_short_name(name: str | None) -> str | None:
    ...
    if len(short_name) > 40 and ":" in short_name:
        ...
        is_time_colon = (
            before_colon
            and before_colon[-1].isdigit()
            and after_colon
            and after_colon[0].isdigit()
        )
    ...
    short_name = re.sub(
        r"\s+[-–]\s+.*\b(?:20\d{2}|at\s+\w).*$", "", short_name, flags=re.IGNORECASE
    )
    ...
    short_name = re.sub(
        rf"\s+{months}\s+\d{{1,2}}(?:st|nd|rd|th)?\s+(.+?)\s+\d{{1,2}}:\d{{2}}\s*(?:am|pm|AM|PM)?$",
        r" \1",
        short_name,
        flags=re.IGNORECASE,
    )
    short_name = re.sub(
        rf"\s+{months}\s+\d{{1,2}}(?:st|nd|rd|th)?(?:\s+\d{{1,2}}:\d{{2}}\s*(?:am|pm|AM|PM)?)?$",
        "",
        short_name,
        flags=re.IGNORECASE,
    )
    ...
    short_name = re.sub(
        rf"\s*[-–]\s*{days_short}(?:\s+\d{{1,2}}:\d{{2}}\s*(?:am|pm)|\s*[-–]\s*\d{{1,2}}:\d{{2}}\s*(?:am|pm))?(?:\s*[-–]\s*{months})?$",
        "",
        short_name,
        flags=re.IGNORECASE,
    )
    ...
    short_name = re.sub(
        r"\s+\d{1,2}:\d{2}\s*(?:am|pm)$", "", short_name, flags=re.IGNORECASE
    )
...
def process_tags(
    ...
    if isinstance(hashtags_field, list):
        ...
        raw_tags = [
            tag.strip().rstrip(",") for tag in hashtags_field.split("#") if tag.strip()
        ]
    ...
    for tag in raw_tags:
        ...
        final_tag = re.sub(
            r"(?<!^)\b(A|And|Of|The|Or|In|At|On|For|To|With|From|By)\b",
...
def filter_by_tag(processed_row: dict[str, Any], tag_rules: dict[str, Any]) -> bool:
    """Filters a row based on removable tags."""
    tags_to_remove = set(tag_rules.get("remove", []))
    event_tags = set(
        tag.lower().replace(" ", "") for tag in processed_row.get("tags", [])
    )
...
def filter_by_date(
    row_dict: dict[str, Any], current_date: date, future_limit_date: date
) -> bool:
...
def _standardize_time(time_str: str) -> str:
...
def normalize_event_name_caps(event_name: str) -> str:
    ...
    two_letter_acronyms = {
        m for m in re.findall(r"\b([A-Z]{2})\b", event_name) if m not in common_words
    }
    ...
    event_name = re.sub(
        r"(\d+)(St|Nd|Rd|Th)\b", lambda m: m.group(1) + m.group(2).lower(), event_name
    )
    event_name = re.sub(
        r"\b([BCDFGHJKLMNPQRSTVWXYZ])([bcdfghjklmnpqrstvwxyz])\b",
...
def group_event_occurrences(
    rows: list[dict[str, Any]], source_url: str | None = None
) -> list[dict[str, Any]]:
    ...
    def find_matching_group_key(
        event_name: str, grouped_events: dict[str, dict[str, Any]]
    ) -> str:
        ...
        for existing_key in grouped_events.keys():
            ...
            if len(normalized_event) >= 5 and len(normalized_existing) >= 5:
                if (
                    normalized_event in normalized_existing
...
def _normalize_location_name(name: str | None) -> str:
    ...
    _boroughs: list[str] = []
    has_dash_before_borough: bool = any(
        f"- {b}" in original_lower or f"_{b}" in original_lower for b in _boroughs
    )
...
def _normalize_street_address(address_str: str | None) -> str | None:
...
def _extract_street_address(full_address: str | None) -> str | None:
...
def build_locations_map(cursor: Any) -> dict[str, Any]:
    ...
    def add_with_duplicates(
        tier: dict[str, Any], key: str, full_info: dict[str, Any]
    ) -> None:
...
def build_websites_map(cursor: Any) -> dict[str, list[str]]:
...
def get_location_id(
    ...
    def make_result(info: dict[str, Any]) -> dict[str, Any]:
    ...
    def get_first(match: Any) -> dict[str, Any]:
        """Get first item if list, otherwise return as-is."""
        result: dict[str, Any] = match[0] if isinstance(match, list) else match
        return result
    ...
    if len(full_loc) > 3 or len(normalized_name) > 3:
        for priority, tier in all_tiers:
            for key in tier:
                ...
                is_match = (
                    key == normalized_loc
                    or (len(normalized_name) > 3 and key == normalized_name)
                    or (
                        len(key) > 3
                        and (
                            full_loc.startswith(key)
                            or full_loc.endswith(key)
                            or key in full_loc
                        )
                    )
                    or (len(normalized_loc) > 3 and normalized_loc in key)
                    or (len(normalized_subloc) > 3 and normalized_subloc in key)
                )
                ...
                if is_match:
                    if len(normalized_name) > 3 and key == normalized_name:
                        ...
                        full_loc.startswith(key) or full_loc.endswith(key)
                    ):
                        ...
                        score = max(
                            ...
                            _calculate_levenshtein_ratio(normalized_name, key)
                            if len(normalized_name) > 3
                            else 0,
                        )
    ...
    for priority, tier in all_tiers:
        for key in tier:
            ...
            if isinstance(match, list):
                continue
            score = _calculate_levenshtein_ratio(
                normalized_site, _normalize_location_name(key)
            )
            if score >= 0.85 and (
                score > best_score or (score == best_score and priority < best_priority)
            ):
...
def extract_url_from_content(content: str | None) -> tuple[str | None, str | None]:
...
def _parse_json_events(extracted_content: str) -> list[dict[str, Any]] | None:
...
def _parse_markdown_table(extracted_content: str) -> list[dict[str, Any]]:
...
def process_events(
    ...
    source_url, _ = (
        extract_url_from_content(crawled_content) if crawled_content else (None, None)
    )
    ...
    for row_dict in parsed_rows:
        ...
        if any(
            kw in processed_row.get("location", "").lower()
            for kw in ["virtual", "online", "livestream"]
        ):
