...
def get_db_config() -> dict[str, str]:
    ...
    env: str = os.environ.get("FOMO_ENV", "local")
    if env not in DB_CONFIG:
        env = "local"
    return DB_CONFIG[env]


def create_connection() -> Any | None:
...
def _parse_url_data(url_string: str) -> list[dict[str, str | None]]:
    ...
    for item in url_string.split("|||"):
        ...
        url: str = parts[0]
        js_code: str | None = parts[1] if len(parts) > 1 and parts[1] else None
        urls.append({"url": url, "js_code": js_code})
    return urls


def get_websites_due_for_crawling(
    cursor: Any, website_ids: list[int] | None = None
) -> list[dict[str, Any]]:
    """
...
def get_or_create_crawl_run(cursor: Any, connection: Any, run_date: str) -> int:
...
def create_crawl_result(
    cursor: Any, connection: Any, crawl_run_id: int, website_id: int, filename: str
) -> int:
...
def update_crawl_result(
    cursor: Any, connection: Any, crawl_result_id: int, status: str, **kwargs: Any
) -> None:
    """
...
def update_crawl_result_crawled(
    cursor: Any, connection: Any, crawl_result_id: int, content: str
) -> None:
    """Update crawl result with crawled content."""
    update_crawl_result(cursor, connection, crawl_result_id, "crawled", content=content)


def update_crawl_result_extracted(
    cursor: Any, connection: Any, crawl_result_id: int, content: str
) -> None:
...
def update_crawl_result_processed(
    cursor: Any, connection: Any, crawl_result_id: int, event_count: int
) -> None:
...
def update_crawl_result_failed(
    cursor: Any, connection: Any, crawl_result_id: int, error_message: str | None
) -> None:
...
def update_website_last_crawled(cursor: Any, connection: Any, website_id: int) -> None:
...
def complete_crawl_run(cursor: Any, connection: Any, crawl_run_id: int) -> None:
...
def get_incomplete_crawl_results(cursor: Any) -> list[dict[str, Any]]:
    """
...
def get_crawled_content(cursor: Any, crawl_result_id: int) -> str | None:
...
def get_existing_upcoming_events(cursor: Any, website_id: int) -> list[dict[str, Any]]:
    """
...
def archive_outdated_events(
    cursor: Any, connection: Any, website_id: int
...
def get_extracted_content(
    cursor: Any, crawl_result_id: int
...
def get_all_locations(cursor: Any) -> list[dict[str, Any]]:
    """
...
def get_tag_rules(cursor: Any) -> dict[str, dict[str, str] | list[str]]:
    """
    ...
    for row in cursor.fetchall():
        ...
        if rule_type == "rewrite":
            rewrite[pattern] = replacement or ""
...
def get_websites_with_tags(cursor: Any) -> dict[str, list[str]]:
    """
